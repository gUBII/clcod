...
def parse_gemini_stream_delta(line: str) -> str | None:
    """Extract assistant text from one Gemini -o stream-json NDJSON line.

    Gemini emits message events with role="assistant" and delta=True that contain
    the streamed content. Returns the content when the line is an assistant delta
    message, None for all other line types (init, user messages, result, empty,
    malformed).
    """
    line = line.strip()
    if not line:
...
def parse_gemini_stream_final(raw: str) -> str:
    """Extract the consolidated reply from Gemini -o stream-json NDJSON output.

    Concatenates content from all assistant delta message events in order.
    """
    parts: list[str] = []
    for line in raw.splitlines():
        text = parse_gemini_stream_delta(line)
        if text is not None:
            parts.append(text)
    return "".join(parts)


def extract_session_id_from_gemini_stream_json(raw: str, fallback: str | None) -> str | None:
    """Extract session_id from Gemini -o stream-json NDJSON output.

    Gemini emits {"type":"init","session_id":"UUID",...} as the first event.
    Returns the first non-empty session_id found, or fallback if none is found.
    """
    for line in raw.splitlines():
        stripped = line.strip()
        if not stripped:
...
PARSERS = {
    ...
    "GEMINI": parse_gemini_stream_final,
}
...
def extract_session_id(agent: dict[str, Any], raw: str, stderr_text: str, session_id: str | None) -> str | None:
    ...
    # Gemini -o stream-json mode: extract session_id from init event in stdout.
    if agent["name"] == "GEMINI":
        json_id = extract_session_id_from_gemini_stream_json(raw, None)
        if json_id:
            return json_id
        return session_id
...
async def call_agent(
    ...
    # ── STREAM_TOKENS path (GEMINI) ───────────────────────────────────────────
    # Gemini uses -o stream-json flag (already in cmd from config), route through
    # _exec_agent_streaming for progressive token delivery.
    if STREAM_TOKENS and agent["name"] == "GEMINI":
        def _on_gemini_stream_line(line: str) -> None:
            if token_callback is None:
                return
            delta = parse_gemini_stream_delta(line)
            if delta:
                token_callback(delta)
        ...
        current_session_id = extract_session_id_from_gemini_stream_json(raw, current_session_id)
        if current_session_id:
            async with session_lock:
                sessions = load_sessions(sessions_path)
                if sessions.get(agent["name"]) != current_session_id:
