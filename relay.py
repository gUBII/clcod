...
def parse_gemini_stream_delta(line: str) -> str | None:
    ...
    Gemini emits {"type":"message","role":"assistant","content":"...","delta":true}
    for each streamed chunk.  Returns the content string, or None for all other
    line types (init, user-echo, result, empty, malformed).
    """
    line = line.strip()
    if not line:
        return None
...
def extract_session_id(agent: dict[str, Any], raw: str, stderr_text: str, session_id: str | None) -> str | None:
    ...
    # Gemini -o stream-json mode: prefer session_id from the init NDJSON event in stdout.
...
async def call_agent(
    ...
    # cmd already contains -o stream-json (from config.json GEMINI args), so no
    # injection needed.  Route through _exec_agent_streaming for progressive
    # token delivery.
